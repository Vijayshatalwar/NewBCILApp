...
{
    /// <summary>
    /// Interaction logic for UCSegragationStackPrinting.xaml
    /// </summary>
    public partial class UCSegragationStackPrinting : UserControl
    {
        ...
        public UCSegragationStackPrinting()
        {
            InitializeComponent();
        }
            ...
            try
            {
                ...
                foreach (PL_SegStackPrinting item1 in lv.ItemsSource)
                {
                    if (item1.IsValid == true)
                    {
                        string objMatCode = Convert.ToString(item1.MatCode);
                        string objUserId = Convert.ToString(item1.UserId);
                        if (DtSelectedItems.Rows.Count != 0 && DtSelectedItems.Rows.Count <= 1)
                        {
                        ...
                        if (VariableInfo.mPlantCode == "2000")
                        {
                            ...
                            DataTable distinctValues = view.ToTable(true, "MatCode");
                        ...
                        else if (sLocationType == "HUB")
                        {
                            if (DtSelectedItems.Rows.Count > 1)
                            {
                ...
                for (int i = 0; i < DtSelectedItems.Rows.Count; i++)
                {
                    ...
                    {
                        dtData = objBLItem.BLSegregationSelectedMatDetails(sLocCode, sMatCode, sUserId);
                ...
                if (dtPostData.Rows.Count > 0)
                {
                    ...
                    if (sStackRunningSerial == string.Empty || sStackRunningSerial == "")
                    {
                        if (Properties.Settings.Default.PrintingSection.Contains("DOOR") && VariableInfo.mPlantCode == "2010")
                            sStackRunningSerial = "40000";
                        else
                            sStackRunningSerial = "0";
                    }
                    ...
                    {
                        oSAPPostCount = 0;
                        ObjLog.WriteLog("Stack QRCode - " + objStackQRCode + " is Printed Successfully");  //+ sSaveCount + " - No. of Records Posted to SAP"
                        BCommon.setMessageBox(VariableInfo.mApp, "Stack QRCode - " + objStackQRCode + " is printed successfully", 2);  //+ sSaveCount + " - no. of records posted to SAP"
                        lv.ItemsSource = null;
                        btnPrint.Cursor = Cursors.Arrow;
                        PLSegStackPrinting = new BUSSINESS_LAYER.BL_SegStackPrinting().BLGetSegregationStackDetails(sLocationType);
                        PLSegStackPrinting = new ObservableCollection<PL_SegStackPrinting>(PLSegStackPrinting.Distinct());
                    ...
                    {
                        BCommon.setMessageBox(VariableInfo.mApp, "Error in segragation stack QRCode printing - " + PrintStatus.ToString(), 2);
                    }
                }
                btnPrint.Cursor = Cursors.Arrow;
            }
        ...
        public string PrintSegregationStackQRCodeItem(string objLocationCode, string objStackQRCode, string sDateFormat, string sPrintingSection, string sLocationType, DataTable dtData)
        {
            try
            {
                ...
                if (OutMsg == "PRINTER READY")
                {
                    for (int i = 0; i < dtData.Rows.Count; i++)
                    {
                        ...
                        {
                            ObjLog.WriteLog(DateTime.Now.ToString("dd-MM-yyyy HH:mm:ss") + " (Details) - " + "PrintSegregationStackQRCodeItem => " + "Barcode - " + objStackQRCode + " Not Update");
                        }
                    }
                    ...
                    ObjLog.WriteLog("PrintSegregationStackQRCode => Total No. of Records -" + distinct.Rows.Count);
                    for (int i = 0; i < distinct.Rows.Count; i++)
                    {
                        sMatCode = distinct.Rows[i][0].ToString().Trim();
                        ObjLog.WriteLog("PrintSegregationStackQRCode => Distinct Table Matcode - " + sMatCode);
                    ...
                    if (OutMsg == "SUCCESS")
                    {
                        ObjLog.WriteLog("Stack QRCode - " + objStackQRCode + " Saved And Printed Successfully");
                ...
                else
                {
                    ...
                    {
                        ObjLog.WriteLog("SegregationStackPrinting => ERROR ~ Segregation Stack Printer IP : " + _bcilNetwork.PrinterIP + " not in network, Kindly Check the network");
                        OutMsg = "ERROR ~ Segregation Stack Printer IP : " + _bcilNetwork.PrinterIP + " not in network, Kindly Check the network";
                        return OutMsg;
                    }
                    else
                    {
                        ObjLog.WriteLog("SegregationStackPrinting => ERROR ~ Segregation Stack Printer IP : " + _bcilNetwork.PrinterIP + " found error, Error is : " + OutMsg);
                        OutMsg = "ERROR ~ Segregation Stack Printer IP : " + _bcilNetwork.PrinterIP + " found error, Error is : " + OutMsg;
                        return OutMsg;
                    }
                }
                return OutMsg;
            ...
            {
                String exDetail = String.Format(ex.Message, Environment.NewLine, ex.Source, ex.StackTrace);
                ObjLog.WriteLog(DateTime.Now.ToString(" (Error) - " + "SegregationStackPrinting => " + exDetail.ToString()));
