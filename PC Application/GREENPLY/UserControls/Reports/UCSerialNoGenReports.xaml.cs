...
namespace GREENPLY.UserControls.Reports
{
    ...
    /// </summary>
    public partial class UCSerialNoGenReports : UserControl
    {
        Logger objLog = new Logger();
        DataTable dt = new DataTable();
        DataTable _dtBindList = null;
        WriteLogFile ObjLog = new WriteLogFile();
        public UCSerialNoGenReports()
        {
            InitializeComponent();
        }
        ...
        private void btngetDetails_Click(object sender, RoutedEventArgs e)
        {
            ...
            catch (Exception ex)
            {
                ...
                ObjLog.WriteLog(" (Error) - " + "SerialNoGenReports => " + exDetail.ToString());
                BCommon.setMessageBox(VariableInfo.mApp, ex.Message, 3);
        ...
        private void DisplayData(PL_Reports _PLPostToSAP)
        {
            try
            {
                PLReports = new BUSSINESS_LAYER.BL_Reports().BLSerialNoGenerationReportData(_PLPostToSAP);
        ...
        //    if (dgShowData.Items.Count > 0)
        //    {
        //        CollectionView view = (CollectionView)CollectionViewSource.GetDefaultView(dgShowData.ItemsSource);
        ...
        //    else
        //    {
        //        return ((item as PLPostToSAP).MaterialCode.IndexOf(txtSearch.Text, StringComparison.OrdinalIgnoreCase) >= 0);
        //    }
        //}
        ...
        private void btnExport_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                if (dgShowData.Items.Count > 0)
                {
                    ...
                    BCommon.setMessageBox("QRCode Serial No. Generation Report", " No Data available For Export", 2);
            }
            catch (Exception ex)
            {
        ...
        public void ReportTypes()
        {
            try
            {
                ...
                _dtBindList.Columns["Quantity"].SetOrdinal(3);
                _dtBindList.Columns["QRCode"].SetOrdinal(4);
                _dtBindList.Columns["StackQRCode"].SetOrdinal(5);
                _dtBindList.Columns["SerialNo"].SetOrdinal(6);
                _dtBindList.Columns["MatStatus"].SetOrdinal(7);
                _dtBindList.Columns["CreatedOn"].SetOrdinal(8);
                ...
                if (BCommon.ExportToCSVFromDataTable(_dtBindList, "", "CSV", "SerialNoGenerationReport"))
                {
                    btnExport.Cursor = Cursors.Arrow;
                    Clear();
                }
