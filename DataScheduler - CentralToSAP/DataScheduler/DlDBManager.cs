...
namespace DATA_ACCESS_LAYER
{
    ...
    public sealed class DBManager : IDBManager, IDisposable
    {
        ...
        public void Open(DataProvider Provider, string conn)
        {
            idbConnection = DBManagerFactory.GetConnection(Provider);
            //idbConnection.ConnectionTimeout = 0;
            idbConnection.ConnectionString = conn;
            if (idbConnection.State != ConnectionState.Open)
                idbConnection.Open();
            this.idbCommand = DBManagerFactory.GetCommand(Provider);
